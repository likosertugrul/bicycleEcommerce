...
- **Animasyonlar:** `globals.css` keyframe'leri (fade-in-up/pop/heart) + `Reveal` (scroll ile beliren, IntersectionObserver + 1.5sn güvenlik ağı). **Ürün kartları ve kategoriler `Reveal` ile sarılı → fold altındakiler kaydırınca belirir** (yükleme anında değil). ProductCard SERVER kalır (`t` sözlüğü fonksiyon içerdiği için client'a serialize edilemez); reveal client `Reveal` sarmalayıcıyla yapılır. Hero giriş + hover lift + rozet/kalp pop. Hepsi `prefers-reduced-motion` uyumlu. `.claude/skills/frontend-design` rehberi kuruldu.

...
3. **Sipariş akışı (Sprint 3 — iyzico HARİÇ) — KURULDU.** `/odeme` checkout (`src/components/checkout-form.tsx`: kargo/mağazadan teslim, giriş yapmışsa adres seçimi / misafirse form, sipariş özeti). `createOrder` (`src/server/order-actions.ts`): toplam SUNUCUDA sepetten hesaplanır, stok kontrol+düşüm, Order+OrderItem+Payment(PENDING, provider=manual), sepet temizlenir → `/siparis/[id]` onay. `src/server/orders.ts` okuma katmanı. Siparişlerim (`/siparislerim`) + admin (`/admin/siparisler` liste + `[id]` durum/kargo yönetimi, server-action formlar). Durum etiketleri `src/lib/order-status.ts`. Kargo: $15 sabit, $500+ ücretsiz, pickup ücretsiz. **iyzico sonra bağlanacak** (şimdilik "Ödeme Bekliyor"; Payment.provider=manual). Checkout metinleri şimdilik TR (i18n sonra).

## Kategori sayfaları & Yasal
- **Kategori:** `/kategori/[slug]` (`BIKE_TYPE_SLUGS` ile tür; geçersiz→404). SEO'lu (generateMetadata + JSON-LD CollectionPage), tür bazlı ürün gridi + `/urunler?tur=` filtre linki. Ana sayfa kutuları + ürün detay breadcrumb + footer artık `/kategori/`'ye gider. Sitemap'te kategori+yasal rotalar var.
- **Yasal sayfalar (TR taslak):** `/gizlilik` (KVKK), `/kullanim-sartlari`, `/mesafeli-satis`, `/cerez-politikasi`, `/iptal-iade`. `src/components/legal-page.tsx` (`LegalPage`+`Section`). Köşeli parantezli firma bilgileri doldurulacak + hukuk incelemesi (sayfada uyarı var). Footer'da linkli.
- **Çerez onayı:** `src/components/cookie-consent.tsx` (client, localStorage `cookie_consent`), root layout'ta. "Kabul Et" ile kapanır.
...
- Ürünlerde `ProductImage` (çoklu, `isCover` + `position`). Admin ürün formu: **çoklu dosya** (`imageFiles`) + **tek tek URL ekleme** (`ImageUrlList` client bileşeni → gizli `imageUrls` inputları, server `getAll`). Düzenleme sayfasında mevcut görseller grid'i: kapak yap/sil + **sıralama ← → (`moveProductImage`)**. Ürün detay: `ProductGallery` (ana + thumbnail + ok; resme tıklayınca **lightbox** — büyük görünüm, ok/klavye ile geçiş, Esc/backdrop kapat). Liste kartlarında `ProductCardImage` ok+nokta. Hero `HeroCarousel` kayan şerit (translateX).
...
- **Toplu görsel içe aktarma:** `public/urunler/`'a foto koy → `node scripts/import-bikes.mjs` (aynı ad+numara → çok görselli tek ürün, numarasız → tekil; TASLAK oluşturur). `node scripts/optimize-images.mjs` → 1600px + WebP q82'ye küçültür (34MB→1.2MB), DB linklerini günceller.
- **Görseller:** `next.config` `images.unoptimized: true` — admin/kullanıcı rastgele URL girdiği için optimizer katmanı kapalı (görseller doğrudan yüklenir; optimizer'ın uzak görseli boyamama sorununu çözer + Cloudflare Images maliyeti yok). Placeholder'lar `public/placeholders/*.svg`.
- **Animasyonlar:** `globals.css` keyframe'leri (fade-in-up/pop/heart) + `Reveal` (scroll ile beliren, IntersectionObserver + 1.5sn güvenlik ağı). **Ürün kartları ve kategoriler `Reveal` ile sarılı → fold altındakiler kaydırınca belirir** (yükleme anında değil). ProductCard SERVER kalır (`t` sözlüğü fonksiyon içerdiği için client'a serialize edilemez); reveal client `Reveal` sarmalayıcıyla yapılır. Hero giriş + hover lift + rozet/kalp pop. **Sayfa geçişleri:** her route grubunda `template.tsx` (`animate-page-in`) → gezinince içerik yumuşak belirir (header/footer layout'ta, sabit). Hepsi `prefers-reduced-motion` uyumlu. `.claude/skills/frontend-design` rehberi kuruldu.
...
- **Görsel yükleme (Supabase Storage):** `images` bucket (public). `src/lib/supabase/admin-client.ts` (service-role) + `src/server/upload.ts uploadImage(file, folder)` → public URL. Admin ürün formu ve ilan formu: dosya yükle VEYA URL (dosya öncelikli). next/image `remotePatterns hostname:"**"` ile tüm https (Storage dahil) yükler.
- **Para birimi: SADECE USD.** `src/lib/currency.ts` USD-only (Currency="USD", RATES.USD=1, dönüşüm yok). Fiyatlar DB'de **USD cent** saklanır. `formatPrice(cents)` → `$X.XX`. Seçici kaldırıldı; ilanlar da USD. (Not: eski TRY-kuruş veriler bir migration script'iyle ×0.025 ile USD cent'e çevrildi. Çoklu-birim altyapısı git geçmişinde var, gerekirse geri getirilebilir.)

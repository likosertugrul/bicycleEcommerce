...
- **Animasyonlar:** `globals.css` keyframe'leri (fade-in-up/pop/heart) + `Reveal` (scroll ile beliren, IntersectionObserver) + hero giriş + kart/kategori stagger + hover lift + rozet/kalp pop. Hepsi `prefers-reduced-motion` uyumlu. `.claude/skills/frontend-design` rehberi kuruldu.
